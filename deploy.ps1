...
# Git workflow script: sync, commit, tag, and push to Working2FA branch
...
if (Test-Path $versionFile) {
    ...
    # Initialize with v1.1.0 for new Working2FA branch
...
if (-not $match) {
    Write-Host "Invalid version format. Expected format: v1.1.x" -ForegroundColor Red
    exit 1
}
...
# Increment patch version (v1.1.x)
